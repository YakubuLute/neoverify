...
export class UploadService {
  ...
  private uploadProgressSubject = new BehaviorSubject<Map<string, DocumentUploadProgress>>(
    new Map()
  );
  ...
  uploadDocument(
    ...
    return this.http
      .post<any>(`${this.apiService.getBaseUrl()}/documents/upload`, formData, {
        reportProgress: true,
        observe: 'events',
      })
      .pipe(
    ...
    return this.apiService.post<BulkIssuanceResult>('documents/bulk-upload', formData).pipe(
      ...
      tap((result) => {
        this.notificationService.success(
          `Bulk upload initiated. Processing ${result.totalItems} documents.`
        );
      }),
      catchError((error) => {
