...
export class DocumentSharingService {
    ...
    shareDocument(request: ShareDocumentRequest): Observable<SharedUser[]> {
        ...
        if (!permissionCheck.allowed) {
            return throwError(() => new Error(permissionCheck.reason || 'No permission to share documents'));
        }

        return this.apiService.post<SharedUser[]>(`documents/${request.documentId}/share`, {
    ...
    createShareLink(request: ShareLinkRequest): Observable<ShareLink> {
        ...
        if (!permissionCheck.allowed) {
            return throwError(() => new Error(permissionCheck.reason || 'No permission to create share links'));
        }

        return this.apiService.post<ShareLink>(`documents/${request.documentId}/share-links`, {
            permissions: request.permissions,
            expiresAt: request.expiresAt,
    ...
    bulkShareDocuments(
        ...
        return this.apiService.post<any>('documents/bulk-share', {
            documentIds,
            userEmails,
            permissions,
            message
    ...
    getDefaultSharePermissions(): SharePermissions {
        ...
        if (!user) {
            return { canView: true, canEdit: false, canDownload: false };
        }
        ...
        switch (user.role) {
            ...
            case 'issuer':
                return { canView: true, canEdit: false, canDownload: true };
            default:
                return { canView: true, canEdit: false, canDownload: false };
        }
    }
